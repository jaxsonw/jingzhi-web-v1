...
const Debugger = ({ generateLoading, fieldList, cueWord, onGenerate }) => {
  const [cueText, setCueText] = useState('')
  const [inputValue, setInputValue] = useState(fieldList)
  const getCueText = value => {
    let newData = value
    const keyList = getInputKeys(newData)
    if (keyList.length === 0) return setCueText(newData)
    keyList.forEach(item => {
      const current = fieldList?.find(val => val.field === item)
      newData = newData?.replaceAll(
        `{{${item}}}`,
        `<span style="background:#4692ff;display:inline-block;color:white; border-radius:5px;  padding:0 5px; font-weight:bold; ">${current?.remark || item
        }</span>`
      )
    })
    setCueText(newData)
    ...
    <div className="relative grow h-full overflow-y-auto  py-4 px-6 bg-gray-50 flex flex-col">
      <div className="shrink-0">
        <div className="flex items-center justify-between mb-2">
          <div className="text-lg text-gray-800 font-bold">调试与预览</div>
        </div>
        <div className="pb-5 border border-gray-200 bg-white rounded-xl ">
          {/* keylist title */}
          <div className="mb-4 px-3">
            <div className="flex items-center space-x-1">
              ...
              <div className="text-sm font-semibold text-gray-800">用户输入</div>
            </div>
            <div className=" py-6">
              <Textarea
                value={inputValue}
          ...
          <div className="flex w-full mt-4 px-5 justify-end">
            <button
              ...
              className="w-[100px] h-[40px] bg-[#3162FF] text-[white] font-bold rounded-xl"
              onClick={onSubmit}
            >
              {generateLoading ? <PulseLoader color="#36d7b7" /> : '运行'}
            </button>
          </div>
        </div>
      </div>
