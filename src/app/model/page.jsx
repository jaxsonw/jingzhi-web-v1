...
const people = [{ name: '全部类型' }, { name: '聊天模型' }, { name: '基础模型' }, { name: '图像模型' }]
    ...
    <div className="h-screen pt-[104px] lg:px-16">
      ...
      <div className="sm:flex pt-[22px]">
        <div className=" sm:flex flex-auto sm:justify-end  sm:flex-grow-[1.78]">
          <button className="w-[120px] h-[40px] mr-[23px] bg-[#3162FF] text-white rounded-full">重要参数</button>
          <button className="w-[120px] h-[40px]  bg-[#fff] text-[#1A1A1A] border-2 border-[#98E2FF] rounded-full">费用介绍</button>
        </div>
                ...
                <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 pl-4 pr-4 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm border-1 border-[#3162FF]">
                  {people.map((person, personIdx) => (
                    ...
                    >
                      {({ selected }) => (
                        <>
                          <span className={`block truncate ${selected ? 'text-[#3162FF]' : 'font-normal'}`}>{person.name}</span>
                          {/* {selected ? (
